...
def printTag(word, tagTekst):
    ...
    if re.search(multiwordExprPattern, wordWithoutDollar):
        ...
        for wordElem, lemma, idx in zip(wordElemList, lemmaList, indices):
            ...
            # with open(UTFIL, 'a', encoding='utf-8') as tag_utfil:
            #     tag_utfil.write('"<{minWord}>"\n{tagTekstUttrykk}'.format(**vars()))
            OUTPUT_RESULT_LIST.append('"<{minWord}>"\n{tagTekstUttrykk}'.format(**vars()))
    else:
        # if WXML:
        ...
        # with open(UTFIL, 'a', encoding='utf-8') as tag_utfil:
        #     tag_utfil.write('"<{wordLower}>"\n{tagTekst}'.format(**vars()).format(**vars()))
        OUTPUT_RESULT_LIST.append('"<{wordLower}>"\n{tagTekst}'.format(**vars()).format(**vars()))
